...
def update_lifetime(_a, _b):
    return False
tau_do = [[5,5,5],[5,5,5]]
atmosphere = NCell(S_i, S_di, D_i, [N_i1, N_i2], target_alts, [alt1, alt2], [dh, dh], lam, drag_lifetime, update_lifetime, tau_do=tau_do)

atmosphere.run_sim_precor(T, upper=False)
atmosphere.save('./', 'test_save_2CellNTypeDeorbit', gap=0.01, compress=True)
